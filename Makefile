...
B = $(shell cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1)
C = $(shell cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 16 | head -n 1)

all:
	make -C /lib/modules/$(shell uname -r)/build M=$(PWD) modules 
		
mount:
	insmod the_reference_monitor.ko the_syscall_table=$(A) keystream=$(B) iv=$(C)
